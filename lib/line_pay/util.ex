defmodule LinePay.Util do
  ...
  def handle_line_pay_response(res) do
    cond do
      res["error"] -> {:error, res}
      res["data"] -> {:ok, Enum.map(res["data"], &LinePay.Util.string_map_to_atoms(&1))}
      true -> {:ok, LinePay.Util.string_map_to_atoms(res)}
    end
  end

  # returns the full response in {:ok, response}
  # this is useful to access top-level properties
  def handle_line_pay_full_response(res) do
    cond do
      res["error"] -> {:error, res}
      true -> {:ok, LinePay.Util.string_map_to_atoms(res)}
    end
  ...
  def list_raw(endpoint, opts \\ []) do
    ...
    list_raw(endpoint, LinePay.config_or_env_key(), opts)
  end

  def list_raw(endpoint, key, opts) do
    q = "#{endpoint}?#{opts |> Enum.filter(fn {_, v} -> v end) |> URI.encode_query()}"

    LinePay.make_request_with_key(:get, q, key)
    |> LinePay.Util.handle_line_pay_full_response()
  end
  ...
  def list(endpoint, opts \\ []) do
    ...
    list(endpoint, LinePay.config_or_env_key(), opts)
  end
end
